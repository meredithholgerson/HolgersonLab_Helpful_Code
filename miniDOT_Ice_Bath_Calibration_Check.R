######################################
# Script for visualizing the results of miniDOT calibration check 
######################################
...
# 3. Extract meta data so that you can connect to serial number 
...
# 4. Add serial number as a column in each data frame and put them all into one big data frame
...
miniDOT_data_trimmed_r1 <- miniDOT_data_r1[miniDOT_data_r1$Date_Time >= start_time_r1 & miniDOT_data_r1$Date_Time <= end_time_r1 , ]

# Round 2 
miniDOT_data_r2 <- miniDOT_data[!miniDOT_data$Serial_Number %in% round1_SNs , ]
start_time_r2 <- as.POSIXct("2023-02-10 9:00:00", tz = "EST", format = "%Y-%m-%d %H:%M")
end_time_r2 <- as.POSIXct("2023-02-10 15:00:00", tz = "EST", format = "%Y-%m-%d %H:%M")
miniDOT_data_trimmed_r2 <- miniDOT_data_r2[miniDOT_data_r2$Date_Time >= start_time_r2 & miniDOT_data_r2$Date_Time <= end_time_r2 , ]
...
miniDOT_calibration_check_plot1  <- miniDOT_data_trimmed_r1 %>%
...
# Round 2 of Calibration (second bucket in first time window)
miniDOT_calibration_check_plot2  <- miniDOT_data_trimmed_r2 %>%
  ggplot(aes(x=Date_Time, y = DOSat_per)) +
  geom_line(aes(y = DOSat_per, color = Serial_Number)) + 
  geom_point(aes(y = DOSat_per, color = Serial_Number)) + 
  theme_bw() +
  ylab("Dissolved Oxygen Saturation (%)") + xlab("Time") + ggtitle("miniDOT Calibration Check: Sheel's Sensors")
miniDOT_calibration_check_plot2 
